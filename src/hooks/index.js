  ...
  useEffect(() => {
    injected.isAuthorized().then(isAuthorized => {
      if (isAuthorized) {
        ...
        if (isMobile && window.zenonnetwork) {
          activate(injected, undefined, true).catch(() => {
            setTried(true)
          })
        } else {
...
export function useInactiveListener(suppress = false) {
  ...
  useEffect(() => {
    const { zenonnetwork } = window

    if (zenonnetwork && zenonnetwork.on && !active && !error && !suppress) {
      ...
      zenonnetwork.on('chainChanged', handleChainChanged)
      zenonnetwork.on('networkChanged', handleNetworkChanged)
      zenonnetwork.on('accountsChanged', handleAccountsChanged)

      return () => {
        if (zenonnetwork.removeListener) {
          zenonnetwork.removeListener('chainChanged', handleChainChanged)
          zenonnetwork.removeListener('networkChanged', handleNetworkChanged)
          zenonnetwork.removeListener('accountsChanged', handleAccountsChanged)
        }
      }
    }
...
export function useTokenContract(tokenAddress, withSignerIfPossible = true) {
  ...
  return useMemo(() => {
    try {
      return getContract(tokenAddress, zts_ABI, library, withSignerIfPossible ? account : undefined)
    } catch {
      return null
    }
