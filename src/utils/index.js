...
import ZTS_ABI from '../constants/abis/zts'
import ZTS_BYTES32_ABI from '../constants/abis/zts_bytes32'
import { FACTORY_ADDRESSES, SUPPORTED_THEMES } from '../constants'
import { formatFixed } from '@apuswap/sdk'
...
export function getZenonExplorerLink(networkId, data, type) {
  const prefix = `https://${ZENONEXPLORER_PREFIXES[networkId] || ZENONEXPLORER_PREFIXES[1]}etherscan.io`
...
export function getAllQueryParams() {
  ...
  params.outputCurrency = isAddress(getQueryParam(window.location, 'outputCurrency'))
    ...
    : getQueryParam(window.location, 'outputCurrency') === 'ZNN'
    ? 'ZNN'
    : ''
  ...
  params.znnAmount = !isNaN(getQueryParam(window.location, 'znnAmount'))
    ? getQueryParam(window.location, 'znnAmount')
    : ''
...
export function isAddress(value) {
  try {
    return zennies.utils.getAddress(value.toLowerCase())
  } catch {
    return false
  }
}

export function calculatePlasmaMargin(value, margin) {
  const offset = value.mul(margin).div(zennies.utils.bigNumberify(10000))
...
export function getContract(address, ABI, library, account) {
  if (!isAddress(address) || address === zennies.constants.AddressZero) {
    throw Error(`Invalid 'address' parameter '${address}'.`)
  }

  return new zennies.Contract(address, ABI, getProviderOrSigner(library, account))
}
...
export async function getTokenName(tokenAddress, library) {
  ...
  return getContract(tokenAddress, ZTS_ABI, library)
    .name()
    .catch(() =>
      getContract(tokenAddress, ZTS_BYTES32_ABI, library)
        .name()
        .then(bytes32 => zennies.utils.parseBytes32String(bytes32))
    )
    .catch(error => {
...
export async function getTokenSymbol(tokenAddress, library) {
  ...
  return getContract(tokenAddress, ZTS_ABI, library)
    .symbol()
    .catch(() => {
      const contractBytes32 = getContract(tokenAddress, ZTS_BYTES32_ABI, library)
      return contractBytes32.symbol().then(bytes32 => zennies.utils.parseBytes32String(bytes32))
    })
    .catch(error => {
      error.code = ERROR_CODES.TOKEN_SYMBOL
      throw error
...
// get the zenon balance of an address
export async function getZenonBalance(address, library) {
  if (!isAddress(address)) {
    throw Error(`Invalid 'address' parameter '${address}'`)
  }
...
export function formatZnnBalance(balance) {
...
export async function getTokenBalance(tokenAddress, address, library) {
  ...
  return getContract(tokenAddress, ZTS_ABI, library).balanceOf(address)
}
...
export async function getTokenAllowance(address, tokenAddress, spenderAddress, library) {
  ...
  return getContract(tokenAddress, ZTS_ABI, library).allowance(address, spenderAddress)
}
...
export function amountFormatter(amount, baseDecimals = 18, displayDecimals = 3, useLessThan = true) {
  ...
  // amount > 0
    ...
    // amount of 'sigli' in 1 'zenon'
    const baseAmount = zennies.utils.bigNumberify(10).pow(zennies.utils.bigNumberify(baseDecimals))

    const minimumDisplayAmount = baseAmount.div(
      zennies.utils.bigNumberify(10).pow(zennies.utils.bigNumberify(displayDecimals))
    )

    // if balance is less than the minimum display amount
    if (amount.lt(minimumDisplayAmount)) {
      return useLessThan
        ? `<${zennies.utils.formatUnits(minimumDisplayAmount, baseDecimals)}`
        : `${zennies.utils.formatUnits(amount, baseDecimals)}`
    }
    // if the balance is greater than the minimum display amount
    else {
      const stringAmount = zennies.utils.formatUnits(amount, baseDecimals)
      ...
      // if there is a decimal portion
        ...
        const roundedDecimalComponent = zennies.utils
