...
import { amountFormatter, calculatePlasmaMargin } from '../../utils'
...
function calculateMaxOutputVal(value) {
  if (value) {
    return value.mul(zennies.utils.bigNumberify(10000)).div(ALLOWED_SLIPPAGE.add(zennies.utils.bigNumberify(10000)))
  }
}

function initialAddLiquidityState(state) {
  return {
    inputValue: state.znnAmountURL ? state.znnAmountURL : '',
    outputValue: state.tokenAmountURL && !state.znnAmountURL ? state.tokenAmountURL : '',
...
export default function AddLiquidity({ params }) {
  ...
  const [addLiquidityState, dispatchAddLiquidityState] = useReducer(
    ...
    { znnAmountURL: params.znnAmount, tokenAmountURL: params.tokenAmount, tokenURL: params.token },
    initialAddLiquidityState
  )
  const { inputValue, outputValue, lastEditedField, outputCurrency } = addLiquidityState
  const inputCurrency = 'ZNN'
  ...
  const { reserveZNN, reserveToken } = useExchangeReserves(outputCurrency)
  const isNewExchange = !!(reserveZNN && reserveToken && reserveZNN.isZero() && reserveToken.isZero())
  ...
  const poolTokenPercentage =
    poolTokenBalance && totalPoolTokens && isNewExchange === false && !totalPoolTokens.isZero()
      ...
      : undefined
  const ethShare =
    exchangeZNNBalance && poolTokenPercentage
      ? exchangeZNNBalance
  ...
  const liquidityMinted = isNewExchange
    ? inputValueParsed
    : totalPoolTokens && inputValueParsed && exchangeZNNBalance && !exchangeZNNBalance.isZero()
    ? totalPoolTokens.mul(inputValueParsed).div(exchangeZNNBalance)
    : undefined
  ...
  const ethPerLiquidityToken =
    exchangeZNNBalance && totalPoolTokens && isNewExchange === false && !totalPoolTokens.isZero()
      ? exchangeZNNBalance.mul(zennies.utils.bigNumberify(10).pow(zennies.utils.bigNumberify(18))).div(totalPoolTokens)
      : undefined
  const tokenPerLiquidityToken =
    exchangeTokenBalance && totalPoolTokens && isNewExchange === false && !totalPoolTokens.isZero()
      ? exchangeTokenBalance.mul(zennies.utils.bigNumberify(10).pow(zennies.utils.bigNumberify(18))).div(totalPoolTokens)
      : undefined
  ...
  const marketRate = useMemo(() => {
    return getMarketRate(reserveZNN, reserveToken, decimals)
  }, [reserveZNN, reserveToken, decimals])
  const marketRateInverted = useMemo(() => {
    return getMarketRate(reserveZNN, reserveToken, decimals, true)
  }, [reserveZNN, reserveToken, decimals])
  ...
  function renderTransactionDetails() {
    ...
    if (isNewExchange) {
      return (
        <div>
          <div>
            {t('youAreAdding')} {b(`${inputValue} ZNN`)} {t('and')} {b(`${outputValue} ${symbol}`)} {t('intoPool')}
          </div>
          <LastSummaryText>
            {t('youAreSettingExRate')}{' '}
            {b(
              `1 ZNN = ${amountFormatter(
      ...
      return (
        <>
          <div>
            {t('youAreAdding')} {b(`${amountFormatter(inputValueParsed, 18, 4)} ZNN`)} {t('and')} {'at most'}{' '}
          ...
          <LastSummaryText>
            {t('tokenWorth')} {b(amountFormatter(znnPerLiquidityToken, 18, 4))} ZNN {t('and')}{' '}
  ...
  async function onAddLiquidity() {
    ...
    let znnTransactionSize = (inputValueParsed / 1e18) * 2
    ...
    const plasmaLimit = calculatePlasmaMargin(estimatedPlasmaLimit, PLASMA_MARGIN)

    exchangeContract
      .addLiquidity(
        isNewExchange ? zennies.constants.Zero : liquidityTokensMin,
        isNewExchange ? outputValueParsed : outputValueMax,
        deadline,
        {
          value: inputValueParsed,
  ...
  useEffect(() => {
    if (isNewExchange) {
      ...
      if (outputValue) {
        try {
          const parsedOutputValue = zennies.utils.parseUnits(outputValue, decimals)
  ...
  useEffect(() => {
    if (
      ...
      try {
        const parsedValue = zennies.utils.parseUnits(inputValue, 18)

        if (parsedValue.lte(zennies.constants.Zero) || parsedValue.gte(zennies.constants.MaxUint256)) {
          throw Error()
        }
        ...
        const currencyAmount = marketRate
          .mul(parsedValue)
          .div(zennies.utils.bigNumberify(10).pow(zennies.utils.bigNumberify(18)))
          .div(zennies.utils.bigNumberify(10).pow(zennies.utils.bigNumberify(18 - decimals)))
  ...
  useEffect(() => {
    if (
      ...
      try {
        const parsedValue = zennies.utils.parseUnits(outputValue, decimals)
        ...
        const currencyAmount = marketRateInverted
          .mul(parsedValue)
          .div(zennies.utils.bigNumberify(10).pow(zennies.utils.bigNumberify(decimals)))
    ...
    <>
      ...
      <CurrencyInputPanel
        ...
        extraTextClickHander={() => {
          if (inputBalance) {
            const valueToSet = inputBalance.sub(zennies.utils.parseZenon('.1'))
      ...
      <OversizedPanel hideBottom>
        <SummaryPanel>
          <ExchangeRateWrapper>
            ...
            <span>{marketRate ? `1 ZNN = ${amountFormatter(marketRate, 18, 4)} ${symbol}` : ' - '}</span>
          </ExchangeRateWrapper>
          <ExchangeRateWrapper>
            <ExchangeRate>{t('currentPoolSize')}</ExchangeRate>
            <span>
              {exchangeZNNBalance && exchangeTokenBalance
                ? `${amountFormatter(exchangeZNNBalance, 18, 4)} ZNN + ${amountFormatter(
                    exchangeTokenBalance,
                    decimals,
                    Math.min(4, decimals)
                  )} ${symbol}`
          ...
          <ExchangeRateWrapper>
            ...
            <span>
              {znnShare && tokenShare
                ? `${amountFormatter(znnShare, 18, 4)} ZNN + ${amountFormatter(
                    tokenShare,
                    decimals,
                    Math.min(4, decimals)
                  )} ${symbol}`
